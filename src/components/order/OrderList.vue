<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item :to="{ path: '/' }">首页</el-breadcrumb-item>
      <el-breadcrumb-item>订单管理</el-breadcrumb-item>
      <el-breadcrumb-item>订单列表</el-breadcrumb-item>
    </el-breadcrumb>
    <el-card>
      <el-row :gutter="20">
        <el-col :span="7">
          <el-input
            placeholder="请输入内容"
            v-model="queryInfo.query"
            clearable
            @clear="getOrderList"
          >
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="getOrderList"
            ></el-button>
          </el-input>
        </el-col>
      </el-row>
      <el-table :data="orderList" border stripe>
        <el-table-column type="index"></el-table-column>
        <el-table-column label="订单编号" prop="order_number"></el-table-column>
        <el-table-column
          width="100"
          label="订单价格"
          prop="order_price"
        ></el-table-column>
        <el-table-column width="100" label="是否付款" prop="order_pay">
          <template slot-scope="scope">
            <el-tag type="success" v-if="scope.row.order_pay === '1'"
              >已付款</el-tag
            >
            <el-tag type="danger" v-else>未付款</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          width="120"
          label="是否发货"
          prop="is_send"
        ></el-table-column>
        <el-table-column width="280" label="下单时间">
          <template slot-scope="scope">
            {{ scope.row.create_time | dateFormat }}
          </template>
        </el-table-column>
        <el-table-column label="操作">
          <el-button
            type="primary"
            icon="el-icon-edit"
            @click="addressDialogVisble = true"
          ></el-button>
          <el-button
            type="success"
            icon="el-icon-location"
            @click="showProgressBox"
          ></el-button>
        </el-table-column>
      </el-table>
      <el-pagination
        @size-change="handleSizeChange"
        @current-change="handleCurrentChange"
        :current-page="queryInfo.pagenum"
        :page-sizes="[5, 10, 15, 20]"
        :page-size="queryInfo.pagesize"
        layout="total, sizes, prev, pager, next, jumper"
        :total="total"
      >
      </el-pagination>
    </el-card>
    <!-- 修改定位信息 -->
    <el-dialog
      title="修改地址"
      :visible.sync="addressDialogVisble"
      width="50%"
      @close="clearAddress"
    >
      <el-form
        :model="addressForm"
        :rules="addressFormRules"
        ref="addressFormRef"
      >
        <el-form-item label="省市区/县" label-width="120px" prop="address1">
          <el-cascader
            v-model="addressForm.address1"
            :options="citydata"
            :props="cascaderProps"
            clearable
            style="width: 100%"
          ></el-cascader>
        </el-form-item>
        <el-form-item label="详细地址" label-width="120px" prop="address2">
          <el-input v-model="addressForm.address2"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="addressDialogVisble = false">取 消</el-button>
        <el-button type="primary" @click="addressDialogVisble = false"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    <!-- 查看物流 -->
    <el-dialog
      title="查看物流信息"
      :visible.sync="progressDialogVisble"
      width="50%"
    >
      <el-timeline>
        <el-timeline-item
          v-for="(activity, index) in progressInfo"
          :key="index"
          :timestamp="activity.time"
        >
          {{ activity.context }}
        </el-timeline-item>
      </el-timeline>
      <span slot="footer" class="dialog-footer">
        <el-button @click="progressDialogVisble = false">取 消</el-button>
        <el-button type="primary" @click="progressDialogVisble = false"
          >确 定</el-button
        >
      </span>
    </el-dialog>
  </div>
</template>
<script>
import citydata from './citydata'
import progressInfo from './progressInfo'
export default {
  data () {
    return {
      orderList: [],
      queryInfo: {
        query: '',
        pagenum: 1,
        pagesize: 10
      },
      total: 0,
      citydata,
      addressDialogVisble: false,
      addressForm: {
        address1: [],
        address2: ''
      },
      addressFormRules: {
        address1: [
          { required: true, message: '请选择省市区/县', trigger: 'blur' }
        ],
        address2: [
          { required: true, message: '请输入详细地址', trigger: 'blur' }
        ]
      },
      cascaderProps: {
        expandTrigger: 'hover',
        value: 'value',
        label: 'value',
        chidren: 'chidren'
      },
      progressDialogVisble: false
    }
  },
  created () {
    this.getOrderList()
    // 静态数据
    this.progressInfo = progressInfo.data
  },
  methods: {
    async showProgressBox () {
      this.progressDialogVisble = true
    },
    clearAddress () {
      this.$refs.addressFormRef.resetFields()
      this.addressForm.address1 = []
    },
    handleCurrentChange (num) {
      this.queryInfo.pagenum = num
      this.getOrderList()
    },
    handleSizeChange (size) {
      this.queryInfo.pagesize = size
      this.getOrderList()
    },
    async getOrderList () {
      const { data: res } = await this.$http.get('orders', {
        params: this.queryInfo
      })
      if (res.meta.status !== 200) {
        return this.$message.error('获取订单列表数据失败')
      }
      this.orderList = res.data.goods
      this.total = res.data.total
    }
  },
  filters: {
    dateFormat (val) {
      return new Date(val).toLocaleString()
    }
  }
}
</script>
<style scoped>
</style>
